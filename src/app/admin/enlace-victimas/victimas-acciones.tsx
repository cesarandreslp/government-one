"use client"

import { useActionState } from "react"
import { crearPersonaAction, registrarAhiAction, type VictState } from "./actions"

interface Opcion {
  id: string
  etiqueta: string
}

interface Props {
  puedeAdministrar: boolean
  personas: Opcion[]
}

const inicial: VictState = {}
const INPUT = "w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
const BTN = "rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-60"
const HECHOS = ["DESPLAZAMIENTO", "AMENAZA", "HOMICIDIO", "DESAPARICION_FORZADA", "MINA_ANTIPERSONAL", "OTRO"]

function Mensaje({ state }: { state: VictState }) {
  if (state.error) return <p className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">{state.error}</p>
  if (state.ok) return <p className="mt-2 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700">✅ {state.mensaje}</p>
  return null
}

export function VictimasAcciones({ puedeAdministrar, personas }: Props) {
  const [stPersona, actPersona, pendPersona] = useActionState(crearPersonaAction, inicial)
  const [stAhi, actAhi, pendAhi] = useActionState(registrarAhiAction, inicial)

  if (!puedeAdministrar) return null

  return (
    <div className="grid gap-4 md:grid-cols-2">
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="mb-3 text-sm font-semibold text-slate-700">Nueva persona</h3>
        <form action={actPersona} className="grid gap-2 md:grid-cols-2">
          <select name="tipoDocumento" defaultValue="CC" className={INPUT}>
            <option value="CC">CC</option>
            <option value="CE">CE</option>
            <option value="NIT">NIT</option>
            <option value="PASAPORTE">Pasaporte</option>
          </select>
          <input name="documento" required placeholder="Documento" className={INPUT} />
          <input name="razonSocial" required placeholder="Nombre completo" className={`${INPUT} md:col-span-2`} />
          <button type="submit" disabled={pendPersona} className={`${BTN} md:col-span-2`}>{pendPersona ? "Creando…" : "Crear persona"}</button>
        </form>
        <Mensaje state={stPersona} />
      </div>

      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="mb-3 text-sm font-semibold text-slate-700">Registrar Ayuda Humanitaria Inmediata</h3>
        {personas.length === 0 ? (
          <p className="text-sm text-slate-400">Crea primero una persona.</p>
        ) : (
          <form action={actAhi} className="grid gap-2 md:grid-cols-2">
            <select name="personaId" required defaultValue="" className={`${INPUT} md:col-span-2`}>
              <option value="" disabled>— Persona —</option>
              {personas.map((p) => <option key={p.id} value={p.id}>{p.etiqueta}</option>)}
            </select>
            <select name="hechoVictimizante" required defaultValue="" className={`${INPUT} md:col-span-2`}>
              <option value="" disabled>— Hecho victimizante —</option>
              {HECHOS.map((h) => <option key={h} value={h}>{h.replaceAll("_", " ")}</option>)}
            </select>
            <label className="text-xs text-slate-500">Fecha del hecho<input name="fechaHecho" type="date" required className={INPUT} /></label>
            <label className="text-xs text-slate-500">Fecha de entrega<input name="fechaEntrega" type="date" required className={INPUT} /></label>
            <input name="tipoAyuda" required placeholder="Tipo de ayuda (alimentación, alojamiento, transporte…)" className={`${INPUT} md:col-span-2`} />
            <textarea name="descripcion" rows={2} placeholder="Descripción (opcional)" className={`${INPUT} md:col-span-2`} />
            <button type="submit" disabled={pendAhi} className={`${BTN} md:col-span-2`}>{pendAhi ? "Registrando…" : "Registrar AHI"}</button>
          </form>
        )}
        <Mensaje state={stAhi} />
      </div>
    </div>
  )
}
